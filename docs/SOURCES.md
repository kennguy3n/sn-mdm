...
| `imd`              | `/ibyimd/category/podcasts/` HTML | working — 22 episode permalinks across series; WordPress `/page/N` pagination rejected at discovery. Normaliser anchors on `<div data-elementor-type="wp-post">` so the related-articles widget can't masquerade as the post body (Milestone D fix: previously the first `<article>` was the related-cards widget and 22/25 episodes collapsed to one dedup row). |
| `masters_of_scale` | `episode-sitemap.xml`      | working — first 25 of 700+ permalinks (WordPress sitemap). Normaliser walks `find_all_next` from the `Transcript:` `<h2>` so the verbatim transcript paragraphs are captured even though the heading is nested inside a sticky wrapper `<div>` (Milestone D fix: previously `.next_siblings` only yielded the chevron-toggle UI chrome and 24/25 episodes collapsed to one dedup row). |
