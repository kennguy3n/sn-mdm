...
function sharedPack() {
  ...
  // Register a process-exit hook the first (and only) time we build
  // the shared fixture so the build tempdir is rm'd when the test
  // process exits — including on a failed assertion or uncaught
  // exception. Without this hook the buildDir would leak to OS
  // temp-dir reaping (eventual, but observable in /tmp listings
  // between runs). ``rmSync(..., force: true)`` swallows ENOENT
  // so re-entering this path (shouldn't happen — memoised — but
  // defensively safe) is idempotent.
  if (!_sharedCleanupRegistered) {
    ...
    process.on('exit', () => {
      try {
        ...
        // Best-effort cleanup. ``process.on('exit')`` is synchronous
        // and exceptions here are silently swallowed by Node, but
        // an explicit try/catch makes that intent visible.
      }
    });
  }
